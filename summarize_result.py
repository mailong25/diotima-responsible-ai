...
def main(helm_out_dir, halu_out_dir, output_file):
    # READ results from HELM dataset
    label_tags = [
        'quasi_exact_match', 'chain_of_thought_correctness', 'ifeval_strict_accuracy',
        'exact_match', 'toxic_frac', 'safety_score'
    ]
    test_names = ['simple_safety_tests', 'gpqa', 'mmlu', 'ifeval', 'bbq', 'real_toxicity_prompts']

    predictions = {test: defaultdict(list) for test in test_names}
    ...
    for test in tests:
        ...
        file_path = os.path.join(helm_out_dir, test, 'display_predictions.json')
    ...
    for test_name in predictions:
        ...
        for model in models:
            preds = predictions[test_name][model]
            predictions[test_name][model] = round(sum(preds) / len(preds), 4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--helm_out_dir", type=str, required=True, help="Path to HELM output directory")
    parser.add_argument("--halu_out_dir", type=str, required=True, help="Path to Halu output directory")
    parser.add_argument("--output_file", type=str, default="results.txt", help="Path to save results file")
    
    args = parser.parse_args()
    main(args.helm_out_dir, args.halu_out_dir, args.output_file)